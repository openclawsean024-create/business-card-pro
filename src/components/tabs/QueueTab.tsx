"use client";

import { useMemo } from "react";
import { useStore } from "@/lib/store";
import { FollowupCard } from "../FollowupCard";
import { EmptyState } from "../Common";
import { Users } from "lucide-react";

export function QueueTab() {
  const all = useStore((s) => s.followups);
  const contacts = useStore((s) => s.contacts);
  const completeFollowup = useStore((s) => s.completeFollowup);

  const pending = useMemo(() => {
    return all
      .filter((f) => f.status === "pending")
      .map((f) => ({
        ...f,
        contact: contacts.find((c) => c.id === f.contactId) ?? null,
      }))
      .sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());
  }, [all, contacts]);

  return (
    <section aria-labelledby="queue-h" className="space-y-4">
      <h2 id="queue-h" className="text-2xl font-semibold text-slate-900 flex items-center gap-2">
        <Users className="w-5 h-5 text-brand-600" /> 全部待回訪
        <span className="font-mono text-base text-slate-500 ml-1">({pending.length})</span>
      </h2>
      {pending.length === 0 ? (
        <EmptyState title="沒有待回訪" hint="在『聯絡人』分頁新增即可加入 queue。" />
      ) : (
        <ul className="space-y-2" data-testid="all-queue">
          {pending.map((fu) => (
            <FollowupCard
              key={fu.id}
              followup={fu}
              contact={fu.contact}
              onComplete={completeFollowup}
            />
          ))}
        </ul>
      )}
    </section>
  );
}